 ...
 * specific — so those triggers take `size="compact"` instead (mirrors the same
 * mobile/desktop split as the input height below).
 *
 * Below `sm` (a proxy for touch input, since phones are the only devices this
 * narrow) controls stay at a tap-friendly 36px; `sm` and up — where a mouse is
 * the assumption — they shrink to the original 30px compact size.
 */
export const COMPACT_FIELD_GRID = "gap-x-4 gap-y-2 [&_input]:h-9 sm:[&_input]:h-[30px] [&_label]:text-xs sm:[&_label]:text-[10px]";
